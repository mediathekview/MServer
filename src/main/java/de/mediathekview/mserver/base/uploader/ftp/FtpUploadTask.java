...
public class FtpUploadTask extends UploadTask<FtpUploadTarget> implements CopyStreamListener {
	...
	protected void upload() {
		...
		try (InputStream fileInput = Files.newInputStream(sourcePath)) {
			if (uploadTarget.getPort().isPresent()) {
				client.connect(uploadTarget.getServerUrl(), uploadTarget.getPort().get());
			} else {
				client.connect(uploadTarget.getServerUrl());
			}
			...
			LOG.fatal(String.format("Something went teribble wrong on uploading \"%s\" to \"%s\" via FTP.",
					sourcePath.toString(), uploadTarget.getTargetPath()), ioException);
		}
	...
	@Override
	public void bytesTransferred(final long aTotalBytesTransferred, final int aBytesTransferred,
			final long aStreamSize) {
		try {
