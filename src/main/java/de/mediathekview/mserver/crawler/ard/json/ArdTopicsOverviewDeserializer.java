...
public class ArdTopicsOverviewDeserializer implements JsonDeserializer<Set<CrawlerUrlDTO>> {
  ...
  public Set<CrawlerUrlDTO> deserialize(
    ...
    final JsonObject compilationObject = jsonElement.getAsJsonObject().getAsJsonArray(ELEMENT_WIDGETS).get(0)
  ...
  private Set<CrawlerUrlDTO> parseLetter(final JsonObject letterObject) {
    ...
    for (JsonElement teaserElement :
      ...
      id.ifPresent(s -> results.add(new CrawlerUrlDTO(String.format(ArdConstants.TOPIC_URL, s, ArdConstants.TOPIC_PAGE_SIZE))));
    }
