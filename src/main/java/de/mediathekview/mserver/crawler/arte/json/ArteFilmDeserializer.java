...
public class ArteFilmDeserializer implements JsonDeserializer<Optional<Film>> {
      ...
      if (titleThema.isPresent()) {
        ...
        if (time.isPresent()) {
          ...
          if (baseObject.has(ELEMENT_DURATION_SECONDS)
            ...
            dauer = Duration.of(baseObject.get(ELEMENT_DURATION_SECONDS).getAsLong(),
                ChronoUnit.SECONDS);
          } else {
          ...
          return addVideoStreams(baseObject.get(ELEMENT_PROGRAM_ID).getAsString(), film);
        } else {
  ...
  private Optional<Film> addVideoStreams(final String programId, final Film film) {
    final String videoDetailsLink = String.format(VIDEO_STREAM_URL_PATTERN, programId,
        language.getLanguageCode(), language.getLanguageCode().toLowerCase());
    final Optional<ArteVideoDetailDTO> videoDetails = gatherVideoDetails(videoDetailsLink);
    if (videoDetails.isPresent()) {
      videoDetails.get().getUrls().entrySet().forEach(videoDetailUrl -> film
          .addUrl(videoDetailUrl.getKey(), CrawlerTool.uriToFilmUrl(videoDetailUrl.getValue())));
