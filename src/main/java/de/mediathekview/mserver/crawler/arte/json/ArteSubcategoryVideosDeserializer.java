...
public class ArteSubcategoryVideosDeserializer implements JsonDeserializer<ArteFilmListDTO> {
  private static final Logger LOG = LogManager.getLogger(ArteSubcategoryVideosDeserializer.class);
  ...
  public ArteFilmListDTO deserialize(final JsonElement aJsonElement, final Type aType,
    ...
    if (aJsonElement.isJsonObject()) {
      ...
      if (JsonUtils.checkTreePath(mainObj, Optional.of(crawler), JSON_ELEMENT_DATA)) {
        mainObj.get(JSON_ELEMENT_DATA).getAsJsonArray().forEach(result::addFoundFilm);
      }
    }
    return result;
  }
  ...
  private Optional<URI> getNextPageLink(final JsonObject mainObj) {
    if (JsonUtils.checkTreePath(mainObj, Optional.empty(), JSON_ELEMENT_META, JSON_ELEMENT_DATA,
      ...
      final String nextPageUrl = mainObj.get(JSON_ELEMENT_META).getAsJsonObject()
          .get(JSON_ELEMENT_DATA).getAsJsonObject().get(JSON_ELEMENT_LINKS).getAsJsonObject()
