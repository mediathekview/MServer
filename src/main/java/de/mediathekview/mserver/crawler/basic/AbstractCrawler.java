 ...
 */
public abstract class AbstractCrawler implements Callable<CrawlerResult> {
	...
	protected RecursiveTask<Set<AbstractMediaResource<?>>> crawlerTask;
	...
	public CrawlerResult call() {
		...
		updateProgress();
		crawlerTask = createCrawlerTask();
		convertToCrawlerResult(forkJoinPool.invoke(crawlerTask));
	...
	private void convertToCrawlerResult(Set<AbstractMediaResource<?>> mediaResources) {
		for (AbstractMediaResource<?> resource : mediaResources) {
			if (resource instanceof Film) {
				crawlerResult.addFilm((Film) resource);
			} else if (resource instanceof Podcast) {
				crawlerResult.addPodcast((Podcast) resource);
			} else if (resource instanceof Livestream) {
				crawlerResult.addLivestream((Livestream) resource);
			} else {
				throw new IllegalStateException("Found a media resource which is not part of the crawler result.");
			}
		}
	}
	 ...
	 */
	protected abstract RecursiveTask<Set<AbstractMediaResource<? extends MediaUrl>>> createCrawlerTask();
