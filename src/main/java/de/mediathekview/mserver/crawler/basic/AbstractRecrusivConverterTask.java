 ...
 * {@link AbstractRecrusivConverterTask#processElement(Object)} for each.
 *
...
public abstract class AbstractRecrusivConverterTask<T, D> extends RecursiveTask<Set<T>> {
	...
	public AbstractRecrusivConverterTask(final AbstractCrawler aCrawler,
	...
	protected Set<T> compute() {
		if (elementsToProcess.size() <= getMaxElementsToProcess()) {
			...
			final AbstractRecrusivConverterTask<T, D> rightTask = createNewOwnInstance(createSubSet(elementsToProcess));
			final AbstractRecrusivConverterTask<T, D> leftTask = createNewOwnInstance(elementsToProcess);
			leftTask.fork();
	 ...
	 *
	 * @param aElementsToProcess The {@link ConcurrentLinkedQueue} of {@link D} the
	 *                           new instance should process.
