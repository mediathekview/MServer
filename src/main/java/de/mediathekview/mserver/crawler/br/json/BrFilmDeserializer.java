...
public class BrFilmDeserializer implements JsonDeserializer<Optional<Film>> {
	 ...
	 * <code>data {@literal -> } viewer {@literal -> } clip {@literal -> } videoFiles {@literal -> } edges[] {@literal -> } node {@literal -> } id</code><br>
	 ...
	 * <code>data {@literal -> } viewer {@literal -> } detailClip {@literal -> } broadcasts {@literal -> } edges[0] {@literal -> } node {@literal -> } start</code><br>
	 * Optional:
	 * <code>data {@literal -> } viewer {@literal -> } detailClip {@literal -> } shortDescription</code><br>
	 * Optional:
	 * <code>data {@literal -> } viewer {@literal -> } detailClip {@literal -> } description</code>
	 */
	@Override
	...
	private Optional<Film> createFilm(final JsonObject aDetailClip) {
		...
		if (aDetailClip.has(JSON_ELEMENT_TITLE) && aDetailClip.has(JSON_ELEMENT_KICKER)
			...
			try {
				final Film film = new Film(UUID.randomUUID(), crawler.getSender(), title, thema, time, duration);
