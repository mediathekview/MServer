...
public class BrFilmDeserializer implements JsonDeserializer<Optional<Film>> {
   ...
   * The data has this structure: <code>
   * data {@literal -> } viewer {@literal -> } clip {@literal -> } videoFiles {@literal -> } edges[] {@literal -> } node {@literal -> } id
   * </code><br>
   * <code>data {@literal -> } viewer {@literal -> } detailClip {@literal -> } title</code><br>
   * <code>data {@literal -> } viewer {@literal -> } detailClip {@literal -> } kicker</code><br>
   * <code>data {@literal -> } viewer {@literal -> } detailClip {@literal -> } duration</code><br>
   * <code>
   * data {@literal -> } viewer {@literal -> } detailClip {@literal -> } broadcasts {@literal -> } edges[0] {@literal -> } node {@literal -> } start
   * </code><br>
   * Optional: <code>
   * data {@literal -> } viewer {@literal -> } detailClip {@literal -> } shortDescription</code><br>
   * Optional: <code>data {@literal -> } viewer {@literal -> } detailClip {@literal -> } description
   * </code>
   */
  @Override
  public Optional<Film> deserialize(
      final JsonElement aElement, final Type aType, final JsonDeserializationContext aContext) {
    try {
  ...
  private boolean addUrls(final Optional<Film> aNewFilm, final JsonObject viewer) {
    ...
    if (aNewFilm.isPresent() && !urls.isEmpty()) {
      ...
      final List<BrUrlDTO> bestUrls =
          urls.stream()
              .sorted(Comparator.comparingInt(BrUrlDTO::getWidth).reversed())
    ...
    if (detailClip.isPresent()) {
      ...
      if (newFilm.isPresent() && subtitleUrl.isPresent()) {
        try {
          ...
          LOG.fatal(ERROR_VIDEO_URL, malformedURLException);
          crawler.printMessage(
              ServerMessages.DEBUG_INVALID_URL, crawler.getSender().getName(), subtitleUrl.get());
        }
      }
  ...
  private Optional<Film> createFilm(final JsonObject aDetailClip) {
    ...
    if (aDetailClip.has(JSON_ELEMENT_TITLE)
        && aDetailClip.has(JSON_ELEMENT_KICKER)
        ...
        if (videoProfile.has(JSON_ELEMENT_ID)) {
          if (videoProfile.has(JSON_ELEMENT_WIDTH)) {
            if (!videoProfile.get(JSON_ELEMENT_WIDTH).isJsonNull()
              ...
              return Optional.of(
                  new BrUrlDTO(
                      aNode.get(JSON_ELEMENT_PUBLIC_LOCATION).getAsString(),
  ...
  private LocalDateTime toTime(final String aStart) {
    final LocalDateTime local = LocalDateTime.parse(aStart, DateTimeFormatter.ISO_OFFSET_DATE_TIME);
    final ZonedDateTime zoned = local.atZone(ZONE_ID);
    final int hoursToAdd = zoned.getOffset().getTotalSeconds() / 3600;
