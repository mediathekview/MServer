...
public class BrSendungenMissedTask implements Callable<Set<String>> {
  ...
  private static final String API_URL = "https://proxy-base.master.mango.express/graphql";
  private static DateTimeFormatter BR_FORMATTER = DateTimeFormatter.ISO_DATE;
  private final AbstractCrawler crawler;
  private final MServerBasicConfigDTO config;

  public BrSendungenMissedTask(final AbstractCrawler aCrawler) {
    crawler = aCrawler;
    config = MServerConfigManager.getInstance().getConfig(aCrawler.getSender());
  }

  public static void main(final String[] args) throws Exception {
    new BrSendungenMissedTask(null).call();
  }
  ...
  public Set<String> call() throws Exception {
    ...
    final String fromDateString = LocalDateTime.now().plus(3, ChronoUnit.DAYS).format(BR_FORMATTER);
    // 2017-09-22T04:00:00.000Z
    final String toDateString =
        LocalDateTime.now().minus(config.getMaximumDaysForSendungVerpasstSection(), ChronoUnit.DAYS)
            .format(BR_FORMATTER);
