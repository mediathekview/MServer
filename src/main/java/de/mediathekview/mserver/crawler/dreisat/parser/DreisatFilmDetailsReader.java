...
public class DreisatFilmDetailsReader {
  ...
  private static final String API_URL_PATTERN = "%s/tmd/2/ngplayer_2_3/vod/ptmd/3sat/%s/%d";
  ...
  public DreisatFilmDetailsReader(
      ...
      if (!titleNodes.isEmpty()
        ...
        if (downloadInfos.isPresent()) {
          ...
          DownloadDtoFilmConverter.addUrlsToFilm(
              newFilm, downloadInfos.get(), Optional.of(optimizer), ZdfConstants.LANGUAGE_GERMAN);
  ...
  private Optional<DownloadDto> getDownloadInfos(
      final Elements aFilmUrlsApiUrlNodes, final int aStreamVersion) throws IOException {
    final URL apiUrl =
        new URL(
            String.format(
                API_URL_PATTERN, baseUrl, aFilmUrlsApiUrlNodes.get(0).text(), aStreamVersion));
    final Type downloadDtoType = new TypeToken<Optional<DownloadDto>>() {}.getType();
    final Gson gson =
        new GsonBuilder()
            .registerTypeAdapter(downloadDtoType, new ZdfDownloadDtoDeserializer())
            .create();

    try (final InputStreamReader gsonInputStreamReader =
  ...
  private LocalDateTime parseTime(
      final Elements dateNodes,
      final Elements alternativeDateNodes,
      final String thema,
      final String title) {
