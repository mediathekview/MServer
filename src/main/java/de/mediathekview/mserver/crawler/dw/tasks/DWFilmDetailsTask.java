...
public class DWFilmDetailsTask extends AbstractDocumentTask<Film, CrawlerUrlDTO> {
  ...
  private void addDownloadUrls(final CrawlerUrlDTO aUrlDTO, final Film aNewFilm) {
    final String pageId =
    ...
    final WebTarget target =
        ClientBuilder.newClient().target(aUrlDTO.getUrl().replace(pageId, videoId).toString());
    // target.request().g
