...
public class NdrFilmDeserializer {
  ...
  private static Optional<String> parseTopic(final Document aDocument) {
    ...
    if (!topic.isPresent()) {
      final Elements topicElements = aDocument.select(TOPIC2_SELECTOR);

      // sometimes there are several elements of TOPIC2_SELECTOR and the first one is empty
      for (final Element topicElement : topicElements) {
  ...
  private static Optional<String> parseTitle(final Document aDocument) {
    ...
    final Optional<String> episode =
        HtmlDocumentUtils.getElementString(EPISODE_SELECTOR, aDocument);
  ...
  private static Optional<LocalDateTime> parseDate(final Document aDocument) {
    Optional<String> dateTime =
        HtmlDocumentUtils.getElementAttributeString(
            START_DATE_SELECTOR, ATTRIBUTE_CONTENT, aDocument);

    if (!dateTime.isPresent() || dateTime.get().isEmpty()) {
      dateTime =
          HtmlDocumentUtils.getElementAttributeString(TIME_SELECTOR2, ATTRIBUTE_CONTENT, aDocument);
    }
    if (!dateTime.isPresent() || dateTime.get().isEmpty()) {
      dateTime =
          HtmlDocumentUtils.getElementAttributeString(TIME_SELECTOR3, ATTRIBUTE_CONTENT, aDocument);
    }

    if (dateTime.isPresent() && !dateTime.get().isEmpty()) {
      final LocalDateTime localDateTime = LocalDateTime.parse(dateTime.get(), DATE_TIME_FORMATTER);
  ...
  private static Optional<Duration> calculateDuration(final Document aDocument) {
    ...
    final Optional<String> endDate =
        HtmlDocumentUtils.getElementAttributeString(
            END_DATE_SELECTOR, ATTRIBUTE_CONTENT, aDocument);

    if (!startDate.isPresent() || !endDate.isPresent()) {
      return Optional.empty();
    }

    final LocalDateTime startDateTime = LocalDateTime.parse(startDate.get(), DATE_TIME_FORMATTER);
    final LocalDateTime endDateTime = LocalDateTime.parse(endDate.get(), DATE_TIME_FORMATTER);

    final long seconds = ChronoUnit.SECONDS.between(startDateTime, endDateTime);
    return Optional.of(Duration.ofSeconds(seconds));
  }

  private static Optional<Duration> parseDurationStringValueOld(final String[] parts) {
    int index = 0;
  ...
  public Optional<FilmInfoDto> deserialize(final CrawlerUrlDTO aUrlDto, final Document aDocument) {
    ...
    final Optional<String> topic = parseTopic(aDocument);
    final Optional<String> title = parseTitle(aDocument);
    final Optional<String> description =
        HtmlDocumentUtils.getElementAttributeString(
            DESCRIPTION_SELECTOR, ATTRIBUTE_CONTENT, aDocument);
    final Optional<String> videoUrl = parseVideoUrl(aDocument);
    ...
    if (topic.isPresent() && title.isPresent() && videoUrl.isPresent()) {
      final FilmInfoDto dto = new FilmInfoDto(videoUrl.get());
  ...
  private Optional<String> parseVideoUrl(final Document aDocument) {
    ...
    final Optional<String> playerUrl =
        HtmlDocumentUtils.getElementAttributeString(IFRAME_SELECTOR, ATTRIBUTE_SRC, aDocument);
    if (playerUrl.isPresent()) {
      ...
      final int jsonIndex = url.lastIndexOf("?autoplay");
      if (jsonIndex > 0) {
