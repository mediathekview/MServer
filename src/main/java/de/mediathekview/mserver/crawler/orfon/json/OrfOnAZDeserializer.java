...
public class OrfOnAZDeserializer implements JsonDeserializer<PagedElementListDTO<TopicUrlDTO>> {
  ...
  public PagedElementListDTO<TopicUrlDTO> deserialize(
    ...
    //
    PagedElementListDTO<TopicUrlDTO> page = new PagedElementListDTO<TopicUrlDTO>();
    ...
    if (jsonPage.has(TAG_ITEMS[0]) && jsonPage.get(TAG_ITEMS[0]).isJsonObject() &&
      ...
      for (JsonElement topic : jsonPage.get(TAG_ITEMS[0]).getAsJsonObject().get(TAG_ITEMS[1]).getAsJsonArray()) {
        ...
        if (id.isPresent() && url.isPresent()) {
          page.addElement(new TopicUrlDTO(id.get(), url.get()));
        } else {
