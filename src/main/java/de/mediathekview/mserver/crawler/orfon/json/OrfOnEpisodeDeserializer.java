...
public class OrfOnEpisodeDeserializer implements JsonDeserializer<OrfOnVideoInfoDTO> {
  ...
  private static Optional<Resolution> getQuality(final String aQuality) {
    switch (aQuality) {
      ...
      case "QXA", "QXADRM", "QXB", "QXBDRM", "Q8A":
  ...
  public OrfOnVideoInfoDTO deserialize(
    ...
    OrfOnVideoInfoDTO aFilm =
        new OrfOnVideoInfoDTO(
  ...
  private Optional<Map<Resolution, FilmUrl>> optimizeUrls(Optional<Map<Resolution, FilmUrl>> urls) {
    if (urls.isPresent() && urls.get().size() == 1) {
      Map<Resolution, FilmUrl> urlMap = urls.get();
  ...
  private Optional<Set<URL>> buildOrResolveSubs(JsonElement jsonElement) {
    ...
    Optional<JsonElement> embeddedSubtitleSection =
        JsonUtils.getElement(jsonElement, TAG_SUBTITLE_SECTION);
    ...
    if (embeddedSubtitleSection.isPresent()) {
      ...
      try {
        newRequestForSubs =
            crawler.getConnection().requestBodyAsJsonElement(subtitleSource.get(), myMap);
  ...
  private Optional<Set<URL>> parseSubtitleUrls(JsonElement element) {
    ...
    JsonUtils.getElementValueAsString(element, TAG_SUBTITLE_SMI)
        .ifPresent(stringUrl -> toURL(stringUrl).ifPresent(urls::add));
    JsonUtils.getElementValueAsString(element, TAG_SUBTITLE_SRT)
        .ifPresent(stringUrl -> toURL(stringUrl).ifPresent(urls::add));
    JsonUtils.getElementValueAsString(element, TAG_SUBTITLE_TTML)
        .ifPresent(stringUrl -> toURL(stringUrl).ifPresent(urls::add));
    JsonUtils.getElementValueAsString(element, TAG_SUBTITLE_VTT)
        .ifPresent(stringUrl -> toURL(stringUrl).ifPresent(urls::add));
    JsonUtils.getElementValueAsString(element, TAG_SUBTITLE_XML)
        .ifPresent(stringUrl -> toURL(stringUrl).ifPresent(urls::add));
    if (urls.isEmpty()) {
      return Optional.empty();
    }
  ...
  private Optional<Map<Resolution, FilmUrl>> parseVideoFromSegmentPlaylist(
      JsonElement jsonElement) {
    Optional<JsonElement> videoPath1 = JsonUtils.getElement(jsonElement, TAG_VIDEO_PATH_1);
    if (videoPath1.isEmpty()
        || !videoPath1.get().isJsonArray()
        || videoPath1.get().getAsJsonArray().size() == 0) {
      return Optional.empty();
    }

    Optional<Map<String, FilmUrl>> resultingVideos = Optional.empty();
    ...
    if (videoPath1.get().getAsJsonArray().size() == 1) {
      Optional<JsonElement> videoPath2 =
          JsonUtils.getElement(videoPath1.get().getAsJsonArray().get(0), TAG_VIDEO_PATH_2);
    ...
    if (resultingVideos.isPresent()) {
      ...
      if (resultingVideos.get().keySet().contains("QXA")) {
        String url = resultingVideos.get().get("QXA").getUrl().toString();
        try {
  ...
  private Optional<Map<String, FilmUrl>> parseVideoFromSources(JsonElement root) {
    Optional<JsonElement> videoSources = JsonUtils.getElement(root, TAG_VIDEO_FALLBACK);
    if (videoSources.isPresent()) {
      Map<String, FilmUrl> urls = new HashMap<>();
      for (String key : PREFERED_CODEC) {
        ...
        if (codecs.isPresent() && codecs.get().isJsonArray()) {
          for (JsonElement singleVideo : codecs.get().getAsJsonArray()) {
            Optional<String> tgtUrl =
                JsonUtils.getElementValueAsString(singleVideo, TAG_VIDEO_FALLBACK_URL);
            Optional<String> quality =
                JsonUtils.getElementValueAsString(singleVideo, "quality_key");
            if (tgtUrl.isPresent()
                && !tgtUrl.get().contains("/Jugendschutz")
                && !tgtUrl.get().contains("/no_drm_support")
                && !tgtUrl.get().contains("/schwarzung")) {
              try {
                urls.put(quality.get(), new FilmUrl(tgtUrl.get(), 0L));
              } catch (MalformedURLException e) {
    ...
    Optional<Map<String, FilmUrl>> fallbackThumbnail = parseVideoFromThumbnail(root);
    if (fallbackThumbnail.isPresent()) {
      return fallbackThumbnail;
    }
    Optional<Map<String, FilmUrl>> fallbackGapless = parseVideoFromGapless(root);
  ...
  private Optional<Map<String, FilmUrl>> parseVideoFromThumbnail(JsonElement root) {
    Map<String, FilmUrl> urls = new HashMap<>();
  try {
      ...
      if (id.isPresent()
          && thumbnailSources.isPresent()
          && thumbnailSources.get().isJsonArray()
          && thumbnailSources.get().getAsJsonArray().size() > 0) {
        Optional<JsonElement> thumbnailSrc =
            JsonUtils.getElement(thumbnailSources.get().getAsJsonArray().get(0), "src");
        if (thumbnailSrc.isPresent()) {
          int indexId = thumbnailSrc.get().getAsString().indexOf(id.get().getAsString());
          String fromSecondIdOnwards =
              thumbnailSrc
                  .get()
                  .getAsString()
                  .substring(indexId + id.get().getAsString().length() + 1);
          ...
          String url =
              String.format(
                  ...
                  id.get().getAsString(), secondId);
          try {
            urls.put("QXA", new FilmUrl(url, 0L));
          } catch (MalformedURLException e) {
  ...
  private Optional<Map<String, FilmUrl>> parseVideoFromGapless(JsonElement root) {
    Map<String, FilmUrl> urls = new HashMap<>();
    try {
      Optional<JsonElement> gaplessSourceAT =
          JsonUtils.getElement(root, "gapless_sources_austria", "hls");
                ...
                e -> {
                  ...
                  try {
                    if (url.isPresent() && drm.orElse("").equalsIgnoreCase("false")) {
                      urls.put("QXA", new FilmUrl(url.get(), 0L));
                    }
  ...
  private Optional<Map<String, FilmUrl>> readVideoForTargetCodec(
      JsonElement urlArray, String targetCodec) {
    Map<String, FilmUrl> urls = new HashMap<>();
    for (JsonElement videoElement : urlArray.getAsJsonArray()) {
      ...
      if (url.isPresent()
          && codec.isPresent()
          && quality.isPresent()
          && targetCodec.equalsIgnoreCase(codec.get())
          && (OrfOnEpisodeDeserializer.getQuality(quality.get()).isPresent() || quality.orElse("").equalsIgnoreCase("QXA"))) {
        // dummy urls
        if (!url.get().contains("/Jugendschutz")
            && !url.get().contains("/no_drm_support")
            && !url.get().contains("/schwarzung")) {
          try {
            long fileSize = crawler.determineFileSizeInKB(url.get());
            urls.put(quality.get(), new FilmUrl(url.get(), fileSize));
          } catch (MalformedURLException e) {
  ...
  private Optional<LocalDateTime> parseAiredDate(Optional<String> text) {
    ...
    if (text.isPresent()) {
      try {
        result =
            Optional.of(LocalDateTime.parse(text.get(), DateTimeFormatter.ISO_ZONED_DATE_TIME));
      } catch (Exception e) {
