...
public class Wdr4Crawler extends AbstractCrawler {
  
  private static final Logger LOG = LogManager.getLogger(Wdr4Crawler.class);  

  public Wdr4Crawler(ForkJoinPool aForkJoinPool, Collection<MessageListener> aMessageListeners, Collection<SenderProgressListener> aProgressListeners, MServerConfigManager rootConfig) {
  ...
  private Set<TopicUrlDTO> getEntries() throws InterruptedException, ExecutionException {
    ...
    letterPageEntries.addAll(forkJoinPool.submit(radioPageTask).get());
