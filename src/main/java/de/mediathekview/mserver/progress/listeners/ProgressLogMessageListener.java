...
public class ProgressLogMessageListener extends AbstractProgressMessageListener {
  ...
  public void newMessage(final String aMessage) {
    if (lastLog == null
        || Duration.between(lastLog, LocalDateTime.now()).getSeconds()
            >= MIN_SECONDS_BETWEEN_LOGS) {
      LOG.info(aMessage);
