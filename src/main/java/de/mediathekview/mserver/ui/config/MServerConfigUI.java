	...
	private void startScheduling() {
		if (config.hasSchedules()) {
			for (final Entry<String, ScheduleDTO> schedule : config.getSchedules().entrySet()) {
				LOG.info(String.format("Started scheduler %s. The crawlers will run every %d %s", schedule.getKey(),
						schedule.getValue().getDuration(), schedule.getValue().getUnit().name()));
				CrawlerManager.getInstance().getExecutorService().scheduleAtFixedRate(new ScheduleTask(this),
						schedule.getValue().getDuration(), schedule.getValue().getDuration(),
						schedule.getValue().getUnit());
			}
		}
	}
