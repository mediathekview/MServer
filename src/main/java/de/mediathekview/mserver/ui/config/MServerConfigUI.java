...
public final class MServerConfigUI {
  ...
  private void generateDefaultConfiguration() {
    ...
    try {
      final Path defaultConfigFile =
          Paths.get(this.getClass().getClassLoader().getResource(CONFIG_FILE_NAME).toURI());
  ...
  private boolean logLevelInfoOrLower(final Level configLevel) {
    return Level.INFO.equals(configLevel) || Level.DEBUG.equals(configLevel)
  ...
  private void startScheduling() {
    if (config.hasSchedules()) {
      for (final Entry<String, ScheduleDTO> schedule : config.getSchedules().entrySet()) {
        ...
        CrawlerManager.getInstance().getExecutorService().scheduleAtFixedRate(
            new ScheduleTask(this), schedule.getValue().getDuration(),
