...
/**
 * ###########################################################################################################
 * Ablauf: die gefundenen Filme kommen in die "listeFilme" -> bei einem vollen Suchlauf: passiert
 * nichts weiter -> bei einem Update: "listeFilme" mit alter Filmliste auffüllen, URLs die es schon
 * gibt werden verworfen "listeFilme" ist dann die neue komplette Liste mit Filmen
 * ##########################################################################################################
 */
public class FilmeSuchen {

  public static final ListeRunSender listeSenderLaufen = new ListeRunSender(); // Liste mit Infos
                                                                               // über jeden
                                                                               // laufeneden
                                                                               // MedathekReader
  // private
  private final LinkedList<MediathekReader> mediathekListe = new LinkedList<>(); // ist die Liste
                                                                                 // mit allen
                                                                                 // MediathekReadern
                                                                                 // (also allen
                                                                                 // Sender)
  ...
  public ArrayList<String> endeMeldung() {
    ...
    final ArrayList<String> retArray = new ArrayList<>();
    // Sender ===============================================
    // ======================================================
    retArray.add("");
    retArray.add("");
    ...
    // Gesamt ===============================================
    // ======================================================
    final int sekunden = getDauerSekunden();
    retArray.add("");
    retArray
        .add("=================================================================================");
    retArray
    ...
    retArray.add("   Summe geladen[MB]: "
        + RunSender.getStringZaehler(listeSenderLaufen.get(RunSender.Count.SUM_DATA_BYTE)));
    retArray.add("        Traffic [MB]: "
        + RunSender.getStringZaehler(listeSenderLaufen.get(RunSender.Count.SUM_TRAFFIC_BYTE)));

    // Durchschnittswerte ausgeben
    final double doub = 1.0 * listeSenderLaufen.get(RunSender.Count.SUM_TRAFFIC_BYTE)
        / (sekunden == 0 ? 1 : sekunden) / 1000;
    final String rate = doub < 1 ? "<1" : String.format("%.1f", doub);
   ...
   * @param listeFilme
   */
  public synchronized void filmeBeimSenderLaden() {
  ...
  public synchronized RunSender melden(final String sender, final int max, final int progress,
      final String text) {
  ...
  public synchronized void meldenFertig(final String sender) {
    // wird ausgeführt wenn Sender beendet ist
    String zeile;
    final RunSender run = listeSenderLaufen.senderFertig(sender);
    if (run != null) {

      final long anzahlSeiten = listeSenderLaufen.get(run.sender, RunSender.Count.ANZAHL);
      ...
      zeile +=
          "-------------------------------------------------------------------------------------"
              + '\n';
      zeile += "Fertig " + sender + ": " + new SimpleDateFormat("HH:mm:ss").format(new Date())
          + " Uhr, Filme: " + listeSenderLaufen.get(sender, RunSender.Count.FILME) + '\n';
      final int sekunden = run.getLaufzeitSekunden();
    ...
    if (!allStarted || !listeSenderLaufen.listeFertig()) {
      ...
      notifyFertig(new ListenerFilmeLadenEvent(sender, "", listeSenderLaufen.getMax(),
          listeSenderLaufen.getProgress(), (int) listeSenderLaufen.get(RunSender.Count.FILME),
          false));
    }
  }
   ...
   * @param listeFilme
   */
  public void updateSender(final String[] nameSender) {
    // nur für den Mauskontext "Sender aktualisieren"
    boolean starten = false;
    for (final MediathekReader reader : mediathekListe) {
      for (final String s : nameSender) {
  ...
  private synchronized void mrStarten(final int prio) {
    // die MediathekReader mit "prio" starten
    mediathekListe.stream().filter(mr -> mr.getStartPrio() == prio)
        .forEach(mr -> new Thread(mr).start());
  }
  ...
  private void notifyFertig(final ListenerFilmeLadenEvent event) {
    for (final Object l : listeners.getListenerList()) {
  ...
  private void notifyProgress(final ListenerFilmeLadenEvent event) {
    for (final Object l : listeners.getListenerList()) {
      if (l instanceof ListenerFilmeLaden) {
  ...
  private void notifyStart(final ListenerFilmeLadenEvent event) {
    for (final Object l : listeners.getListenerList()) {
      if (l instanceof ListenerFilmeLaden) {
  ...
  private void progressBar() {
    ...
    final int progress = listeSenderLaufen.getProgress();
    int proz = 0;
    String text;
    int sekunden = 0;
    try {
      sekunden =
          Math.round((new Date(System.currentTimeMillis()).getTime() - startZeit.getTime()) / 1000);
    ...
    if (max != 0) {
      ...
      text += " ]  " + listeSenderLaufen.get(RunSender.Count.ANZAHL) + " Seiten / " + proz
          + "% von " + max + " Themen / Filme: " + listeSenderLaufen.get(RunSender.Count.FILME)
          + " / Dauer[Min]: " + (sekunden / 60 == 0 ? "<1" : sekunden / 60) + " / R-Sender: "
          + listeSenderLaufen.getAnzSenderRun();
      Log.progress(text);
    }
  }
}
