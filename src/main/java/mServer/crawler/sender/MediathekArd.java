...
public class MediathekArd extends MediathekReader
{
    ...
    private void addThema()
    {
        ...
        seite = getUrlIo.getUri(SENDER.getName(), ADRESSE_THEMA, StandardCharsets.UTF_8,
    ...
    {
        final GetUrl getUrlIo = new GetUrl(getWartenSeiteLaden());
        seiteFeed = getUrlIo.getUri(SENDER.getName(), strUrlFeed, StandardCharsets.UTF_8,
                2/* max Versuche */, seiteFeed, "");
    ...
    private class ThemaLaden extends Thread
    {
            ...
            for (int i = 0; i <= 6; ++i)
            {
                ...
                while (!Config.getStop() && (pos = seite1.indexOf(MUSTER_ADD_TAGE, pos)) != -1)
                {
                    ...
                    if (!url.isEmpty())
                    {
                        ...
                        datum = seite1.extract("<title>Videos (TV-Sendungen) des Senders Das Erste vom", "- ARD")
                                .trim();
        ...
        private void filmSuchen1(final String strUrlFeed, final String thema, final boolean weiter)
        {
            ...
            if (!Config.getStop() && weiter && (CrawlerTool.loadLongMax() || thema.equalsIgnoreCase("alpha-Centauri")))
            {
                // dann gehts weiter
                int maxWeiter = 0;
                final int maxTh = 10;
        ...
        private void filmSuchen2(final String urlFilm_, final String thema, final String titel, final long dauer,
                final String datum, final String zeit, final String urlSendung)
        {
            // URL bauen:
            // http://www.ardmediathek.de/play/media/21528242?devicetype=pc&features=flash
            try
            {
                ...
                if (!url.isEmpty())
                {
                    ...
                    final Film film = CrawlerTool.createFilm(SENDER, url, titel, thema, datum, zeit, dauer, urlSendung,
                            beschreibung, urlHD, urlKl);
                    if (!subtitle.isEmpty())
                    {
        ...
        private void filmSuchen_old(final String urlSendung, final String thema, final String titel, final long dauer,
                final String datum, final String zeit)
        {
            try
            {
                meldung(urlSendung);
                ...
                {
                    final String beschreibung = beschreibung(urlSendung);
                    addFilm(CrawlerTool.createFilm(SENDER, url, titel, thema, datum, zeit, dauer, urlSendung,
                            beschreibung, "", ""));
                }
                else
                {
