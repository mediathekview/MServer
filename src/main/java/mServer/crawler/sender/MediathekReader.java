...
public abstract class MediathekReader extends Thread {
  ...
  public MediathekReader(final FilmeSuchen aMSearchFilmeSuchen, final String aSendername,
      final int aSenderMaxThread, final int aSenderWartenSeiteLaden, final int aStartPrio) {
  ...
  public synchronized void meldungProgress(final String text) {
    progress++;
  ...
  protected synchronized void meldungStart() {
    ...
    progress = 0;
    Log.sysLog("===============================================================");
    Log.sysLog("Starten[" + (CrawlerTool.loadLongMax() ? "alles" : "update") + "] "
        + getSendername() + ": " + new SimpleDateFormat("HH:mm:ss").format(new Date()));
    Log.sysLog("   maxThreadLaufen: " + getMaxThreadLaufen());
    Log.sysLog("   wartenSeiteLaden: " + getWartenSeiteLaden());
    Log.sysLog("");
    final RunSender runSender =
        mlibFilmeSuchen.melden(getSendername(), getMax(), getProgress(), "" /* text */);
