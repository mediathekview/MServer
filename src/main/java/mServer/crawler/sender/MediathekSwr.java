...
public class MediathekSwr extends MediathekReader
{
    ...
    public final static Sender SENDER = Sender.SWR;

    public MediathekSwr(final FilmeSuchen ssearch, final int startPrio)
    {
        super(ssearch, SENDER.getName(), /* threads */ 2, /* urlWarten */ wartenLang, startPrio);
    }
    ...
    public synchronized void addToList()
    {
        ...
        {
            addToList_verpasst(); // brauchst eigentlich nicht und dauer zu
                                  // lange
        }
        if (Config.getStop())
        {
    ...
    private void addToList__()
    {
        ...
        strSeite = getUrlIo.getUriWithDelay(SENDER.getName(), "http://swrmediathek.de/tvlist.htm",
                StandardCharsets.UTF_8, 2, strSeite, "", 4, TimeUnit.SECONDS);
        int pos = 0;
        String url;
        String thema;
        final int stop = strSeite.indexOf(MUSTER_STOPP);
        while (!Config.getStop() && (pos = strSeite.indexOf(MUSTER_START, pos)) != -1)
        {
            ...
            thema = StringEscapeUtils.unescapeHtml4(thema.trim()); // wird
                                                                   // gleich
                                                                   // benutzt
                                                                   // und muss
                                                                   // dann schon
            ...
            else
            {
                ...
                { "http://swrmediathek.de/tvshow.htm?show=" + url, thema };
                listeThemen.add(add);
            }
        }
    }
    ...
    private void addToList_verpasst()
    {
        ...
        MSStringBuilder strSeite = new MSStringBuilder(Const.STRING_BUFFER_START_BUFFER);
        final GetUrl getUrlIo = new GetUrl(getWartenSeiteLaden());
        strSeite = getUrlIo.getUriWithDelay(SENDER.getName(), "http://swrmediathek.de/sendungverpasst.htm",
                StandardCharsets.UTF_8, 2, strSeite, "", 4, TimeUnit.SECONDS);
        final ArrayList<String> list = new ArrayList<>();
        strSeite.extractList("<ul class=\"progChannelList\" tabindex=\"-1\">", "<div class=\"box mediBoxBorder\"",
                "<a href=\"sendungverpasst.htm?show=&date=", "\"",
                "http://www.swrmediathek.de/sendungverpasst.htm?show=&date=", list);
    ...
    private class ThemaLaden extends Thread
    {
        ...
        public void run()
        {
            try
            {
                ...
                while (!Config.getStop() && themaIterator.hasNext())
                {
                    ...
                    themenSeitenSuchen(thema[0] /* url */, thema[1] /* Thema */);
        ...
        private void themenSeitenSuchen(final String strUrlFeed, final String thema)
        {
            final String MUSTER_URL = "<a href=\"/player.htm?show=";
            // strSeite1 = getUrl.getUri_Utf(nameSenderMReader, strUrlFeed,
            // strSeite1, thema);
            strSeite1 = getUrlThemaLaden.getUriWithDelay(SENDER.getName(), strUrlFeed, StandardCharsets.UTF_8,
                    2 /* versuche */, strSeite1, thema, 4, TimeUnit.SECONDS);
        ...
        private void json(final String strUrlFeed, String thema, final String urlJson)
        {
            ...
            // "entry_title":"\"Troika-Tragödie - Verspielt die Regierung unser
            // Steuergeld?\"
            try
            {
                strSeite2 = getUrlThemaLaden.getUriWithDelay(SENDER.getName(), urlJson, StandardCharsets.UTF_8, 1,
                        strSeite2, "", 4, TimeUnit.SECONDS);
                ...
                final String urldHd = getHDUrl();
                String normalUrl = getNormalUrl();
                String smallUrl = getSmallUrl();
                final String rtmpUrl = getRtmpUrl();
                final String subtitle = strSeite2.extract("\"entry_capuri\":\"", "\"");
                if (thema.isEmpty())
                {
                ...
                else
                {
                    ...
                    final Film film = CrawlerTool.createFilm(SENDER, normalUrl, title, thema, date, time, duration,
                            strUrlFeed, description, urldHd, smallUrl);
                    if (!subtitle.isEmpty())
                    {
        ...
        private String getDate()
        {
            ...
            if (datum.length() < 10)
            {
                ...
                if (datum.indexOf('.') != datum.lastIndexOf('.'))
                {
                    ...
                    {
                        datum = datum.substring(0, datum.indexOf('.') + 1) + '0'
                                + datum.substring(datum.indexOf('.') + 1);
                    }
                }
            }
            return datum;
        ...
        private String getTime()
        {
            ...
            if (time.length() < 8)
            {
                ...
                if (time.indexOf(':') != time.lastIndexOf(':'))
                {
                    ...
                    {
                        time = time.substring(0, time.indexOf(':') + 1) + '0' + time
        ...
        {
            final String PATTTERN_PROT_HTTP_HD =
                    "\"entry_media\",\"attr\":{\"val0\":\"h264\",\"val1\":\"4\",\"val2\":\"http";
            final String urlWithOutprot = strSeite2.extract(PATTTERN_PROT_HTTP_HD, PATTERN_END);
        ...
        {
            final String PATTTERN_PROT_HTTP_L =
                    "\"entry_media\",\"attr\":{\"val0\":\"h264\",\"val1\":\"3\",\"val2\":\"http";
            final String urlWithOutprot = strSeite2.extract(PATTTERN_PROT_HTTP_L, PATTERN_END);
            if (urlWithOutprot.isEmpty())
            {
                return "";
            }
        ...
        {
            final String PATTTERN_PROT_HTTP_M =
                    "\"entry_media\",\"attr\":{\"val0\":\"h264\",\"val1\":\"2\",\"val2\":\"http";
            final String urlWithOutprot = strSeite2.extract(PATTTERN_PROT_HTTP_M, PATTERN_END);
            if (urlWithOutprot.isEmpty())
            {
                return "";
            }
        ...
        {
            final String PATTTERN_PROT_HTTP_S =
                    "\"entry_media\",\"attr\":{\"val0\":\"h264\",\"val1\":\"1\",\"val2\":\"http";
            final String urlWithOutprot = strSeite2.extract(PATTTERN_PROT_HTTP_S, PATTERN_END);
            return HTTP + urlWithOutprot;
        }
