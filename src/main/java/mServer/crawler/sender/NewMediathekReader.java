...
public abstract class NewMediathekReader implements Runnable
{
    private static final Logger LOG = LogManager.getLogger(NewMediathekReader.class);
    ...
    private int maxFilmCount; // Anz. zu suchender Themen
    private int progress; // Prograss eben
    private int startPrio; // es gibt die Werte: 0->startet sofort, 1->später und 2->zuletzt

    private Collection<Film> foundFilme;

    protected GetUrl getUrlIo;
    protected FilmeSuchen mSearchFilmeSuchen;

    public NewMediathekReader(FilmeSuchen aMSearchFilmeSuchen, String aSendername, int aSenderMaxThread, int aSenderWartenSeiteLaden, int aStartPrio)
    {
        foundFilme = Collections.synchronizedList(new ArrayList<>());
    ...
    private synchronized void start()
    {
        ...
        progress = 0;
        LOG.info("===============================================================");
        LOG.info("Starten[" + ((CrawlerTool.loadLongMax()) ? "alles" : "update") + "] " + getSendername() + ": " + new SimpleDateFormat("HH:mm:ss").format(new Date()));
        LOG.info("   maxThreadLaufen: " + getMaxThreadLaufen());
        LOG.info("   wartenSeiteLaden: " + getWartenSeiteLaden());
        RunSender runSender = mSearchFilmeSuchen.melden(getSendername(), getMaxFilmCount(), getProgress(), "" /* text */);
    ...
    {
        maxFilmCount = maxFilmCount + mmax;
        mSearchFilmeSuchen.melden(getSendername(), getMaxFilmCount(), getProgress(), "" /* text */);
    }
