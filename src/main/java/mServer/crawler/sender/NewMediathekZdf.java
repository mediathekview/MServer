...
public class NewMediathekZdf extends NewMediathekReader implements Runnable
{
    ...
    public NewMediathekZdf(FilmeSuchen ssearch, int startPrio)
    {
    ...
    private void addDay()
    {
        ...
        for (long i = 0; i < (CrawlerTool.loadLongMax() ? 300 : 20); ++i)
        {
            ...
            String sendungVerpasstUrl = String.format(URL_PATTERN_SENDUNG_VERPASST, date);
            startUrls.add(sendungVerpasstUrl);


            for(String aKategorieEnd : KATEGORIE_ENDS)
            {
                startUrls.add(String.format(KATEGORIEN_URL_PATTERN,aKategorieEnd));
            }


            listeTage.addUrl(startUrls.toArray(new String[startUrls.size()]));
        }
    }
    ...
    private class ThemaLaden implements Runnable
    {
        ...
        public void run()
        {
            try
            {
                String urls[];
                meldungAddThread();
                while (!Config.getStop() && (urls = listeTage.getListeThemen()) != null)
                {
                    seite1.setLength(0);
                    for (String url : urls)
                    {
                        Log.sysLog("Suche sendungen für Url: " + url);
                        getUrlsDay(url);
                        meldungProgress(url);
                    }
