...
public class ArteJsonObjectToFilmCallable implements Callable<Film> {
  ...
  private static final String ARTE_VIDEO_INFORMATION_URL_PATTERN_2 =
      "https://api.arte.tv/api/opa/v3/programs/%s/%s"; // Für
  ...
  public ArteJsonObjectToFilmCallable(final JsonObject aJsonObjec, final String aLangCode,
    ...
    try {
      if (isValidProgramObject(jsonObject)) {
        ...
        final String videosUrl =
            String.format(ARTE_VIDEO_INFORMATION_URL_PATTERN, langCode, programId);

        final Gson gson = new GsonBuilder()
            .registerTypeAdapter(ArteVideoDTO.class, new ArteVideoDeserializer()).create();
        ...
        try (Response responseVideoDetails = executeRequest(videosUrl)) {
          ...
          LOG.fatal(
              "Beim laden der Informationen eines Filmes für Arte kam es zu Verbindungsproblemen.",
              ioException);
  ...
  private Film createFilm(final String aThema, final String aUrlWeb, final String aTitel,
      final ArteVideoDTO aVideo, final ArteVideoDetailsDTO aArteVideoDetails,
      final Duration aDuration, final String aBeschreibung) throws MalformedURLException {
    ...
    final Film film = new Film(UUID.randomUUID(), sender, aTitel, aThema,
        aArteVideoDetails.getBroadcastBegin(), aDuration);

    film.setGeoLocations(geoLocations);
    film.setWebsite(new URL(aUrlWeb));
  ...
  private boolean isValidProgramObject(final JsonObject programObject) {
    return programObject.has(JSON_ELEMENT_KEY_TITLE)
        && programObject.has(JSON_ELEMENT_KEY_PROGRAM_ID) && programObject.has(JSON_ELEMENT_KEY_URL)
