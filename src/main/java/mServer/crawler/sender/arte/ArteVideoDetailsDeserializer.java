...
public class ArteVideoDetailsDeserializer implements JsonDeserializer<ArteVideoDetailsDTO> {
  ...
  /**
   * Liefert den Beginn der Ausstrahlung, wenn - heute im Zeitraum von
   * CatchUpRights liegt - oder heute vor dem Zeitraum liegt - oder
  ...
  private String getBroadcastDateConsideringCatchupRights(JsonObject broadcastObject) {
    ...
    if (!elementBegin.isJsonNull() && !elementEnd.isJsonNull()) {
      ...
      JsonElement broadcastElement = broadcastObject.get(JSON_ELEMENT_BROADCAST);
      if (broadcastElement != null && !broadcastElement.isJsonNull()) {

        String broadcast = broadcastElement.getAsString();
  ...
  /**
   * *
   * liefert die erste Ausstrahlung des Typs ohne Berücksichtigung der
   * CatchupRights
   */
