...
import mServer.crawler.sender.arte.tasks.ArteVideoDetailDTO;
...
public class ArteVideoDetailsDeserializer
  ...
  public ArteVideoDetailsDeserializer(String aSender) {
    this.sender = aSender;
  }
              ...
              entry -> {
                ...
                if (resolution.isPresent() && arteVideoType.isPresent()) {
                  switch (arteVideoType.get()) {
                    case DEFAULT:
                      arteVideoDTO.put(resolution.get(), url);
                      break;
                    case SUBTITLE_INCLUDED:
                      arteVideoDTO.putSubtitle(resolution.get(), url);
                      break;
                    case AUDIO_DESCRIPTION:
                      arteVideoDTO.putAudioDescription(resolution.get(), url);
                      break;
                    case ORIGINAL_WITH_SUBTITLE:
                      arteVideoDTO.putOriginalWithSubtitle(resolution.get(), url);
                      break;
                    case ORIGINAL:
                      arteVideoDTO.putOriginal(resolution.get(), url);
                      break;
                  }
                }
              });
