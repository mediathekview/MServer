...
public class ZDFIntegrationTest {
  ...
  private JsonObject executeSearch() {
    ...
    final ZonedDateTime minDate = today.minusDays(1);
    final ZonedDateTime maxDate = today.plusDays(0);

    JsonObject jsonData = target.executeSearch(1, minDate, maxDate);
  ...
  private void loadFilm(JsonObject jsonData) {
    ZDFSearchPageTask task =
        new ZDFSearchPageTask(
            jsonData,
            "https://www.zdf.de",
            "https://api.zdf.de",
            "api.zdf.de",
