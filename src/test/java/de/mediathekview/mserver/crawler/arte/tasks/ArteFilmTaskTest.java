...
public class ArteFilmTaskTest extends ArteTaskTestBase {
  ...
  public ArteFilmTaskTest(
    ...
    expectedUrlWithSubtitleSmall = aExpectedUrlWithSubtitleSmall;
    expectedUrlWithSubtitleNormal = aExpectedUrlWithSubtitleNormal;
    expectedUrlWithSubtitleHd = aExpectedUrlWithSubtitleHd;
    expectedUrlAudioDescSmall = aExpectedUrlAudioDescSmall;
    expectedUrlAudioDescNormal = aExpectedUrlAudioDescNormal;
    expectedUrlAudioDescHd = aExpectedUrlAudioDescHd;
  ...
  public void test() {
    ...
    final Iterator<Film> iterator =
        actual.stream().sorted(Comparator.comparing(Film::getTitel)).iterator();
    ...
    if (exceptedFilmCount > 1) {
      ...
      AssertFilm.assertEquals(
          ...
          sender,
          expectedTopic,
          expectedTitle + " (Audiodeskription)",
          expectedTime,
          expectedDuration,
          expectedDescription,
          expectedWebsite,
  ...
  private Set<Film> executeTask(final String aDetailUrl, final String aVideoUrl) {
    final Queue<ArteFilmUrlDto> urls = new ConcurrentLinkedQueue<>();
    urls.add(
        new ArteFilmUrlDto(
            getWireMockBaseUrlSafe() + aDetailUrl, getWireMockBaseUrlSafe() + aVideoUrl));
