...
public class ArteFilmTaskTest extends ArteTaskTestBase {
  ...
  public ArteFilmTaskTest(final Sender aSender, final String aFilmUrl, final String aFilmJsonFile,
  ...
  public void test() {
    ...
    AssertFilm
        .assertEquals(film, sender, expectedTopic, expectedTitle, expectedTime, expectedDuration, expectedDescription, expectedWebsite,
            new GeoLocations[]{expectedGeo}, expectedUrlSmall, expectedUrlNormal, expectedUrlHd, expectedSubtitle);
  }

  private Set<Film> executeTask(final String aDetailUrl, String aVideoUrl) {
    final ConcurrentLinkedQueue<ArteFilmUrlDto> urls = new ConcurrentLinkedQueue<>();
    urls.add(new ArteFilmUrlDto(WireMockTestBase.MOCK_URL_BASE + aDetailUrl, WireMockTestBase.MOCK_URL_BASE + aVideoUrl));
    return new ArteFilmTask(createCrawler(), urls, sender, LocalDateTime.of(2019, 1, 12, 16, 0, 0)).invoke();
  }
}