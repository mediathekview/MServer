...
public class OrfPlaylistDeserializerTest {
  ...
  public void testDeserializeSingleFilm() {
    final JsonElement jsonElement = JsonFileReader.readJson("/orf/orf_playlist_no_episodes.json");

    final OrfPlaylistDeserializer target = new OrfPlaylistDeserializer();
    final List<OrfEpisodeInfoDTO> actual = target.deserialize(jsonElement, null, null);

    assertThat(actual.size(), equalTo(1));

    final OrfEpisodeInfoDTO actualEpisode = actual.get(0);
    assertEpisode(
        actualEpisode,
  ...
  public void testDeserializeFilmWithEpisodes() {
    final JsonElement jsonElement = JsonFileReader.readJson("/orf/orf_playlist_with_episodes1.json");
  ...
  private void assertEpisode(
          final OrfEpisodeInfoDTO actualEpisode,
    ...
    final OrfVideoInfoDTO actualVideoInfo = actualEpisode.getVideoInfo();
    assertThat(actualVideoInfo, notNullValue());

    final Map<Resolution, String> actualVideoUrls = actualVideoInfo.getVideoUrls();
