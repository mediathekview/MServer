...
public class WdrTopicOverviewDeserializerTest {
  ...
  public WdrTopicOverviewDeserializerTest(
    ...
    for (int i = 0; i < aExpectedOverviewUrls.length; i++) {
      expectedUrls[aExpectedFilmUrls.length + i] =
          new WdrTopicUrlDto(aTopic, aExpectedOverviewUrls[i], false);
    }
  }
  ...
  public void deserializeTest() {
    ...
    final WdrTopicOverviewDeserializer target = new WdrTopicOverviewDeserializer();

    final List<WdrTopicUrlDto> actual = target.deserialize(topic, document);
