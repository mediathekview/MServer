...
public class ZdfFilmDetailTaskTestMultipleLanguages extends ZdfTaskTestBase {
  ...
  public void testGermanAndEnglish() {
    ...
    final String[] expectedUrlsSmall = new String[]{
    ...
    final String[] expectedUrlsNormal = new String[]{
    ...
    SortedSet<Film> sortedActual = new TreeSet<>(Comparator.comparing(Film::getTitel));
    sortedActual.addAll(actual);

    for (int i = 0; i < sortedActual.size(); i++) {
      AssertFilm
          .assertEquals(sortedActual.toArray(new Film[]{})[i], Sender.ZDF,
              "Hard Sun", expectedTitles[i],
